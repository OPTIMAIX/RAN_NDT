...
name_scenario_file='ScenarioBSs.mat';
...
config.num_simulations=1;  %Number of simulations to be executed. Each simulation corresponds to a different distribution of relays.
config.simulation_duration=1000;   %s
config.time_step=1; %s Duration of the simulation time step
config.output_file_name='res13BS_F0.mat';
config.map_BS_file_name='Map13BSs.mat';  %Change the name of the file if executing simulations by varying the txpower parameters.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
config.gain_BS=26; %dB  (antenna BS)
config.tx_power=35;  %dBm (ref: Table 7.8-1 of TR38.901)  (old value: 21 dBm)
config.bandwidth=100;  %MHz
config.tx_power_per_Hz=config.tx_power-10*log10(config.bandwidth*1E6); %dBm/Hz
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Parameters of the mobility model (pedestrian and car)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
config.prob_change_inters_cars=0.03;   %When setting this prob. consider that there are multiple opportunities of change during a time_step. The real prob of change is p=1-(1-prob)^N where N is hte number of updates (around 9), so the prob should be set as prob=1-(1-p)^(1/N) where p is the desired change probability
config.speed_cars=30/3.6;         %m/s

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Numbers of users
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
config.num_users=4300;
config.ratio_stationary=0.5;
config.ratio_pedestrians=0.4;
config.ratio_vehicles=0.1;

config.relay_involvement_factor=0; %Fraction of stationary UEs that act as relays
...
config.lambda=zeros(1,4); %s=1:Pedestrians, s=2:stationary non relay, s=3: stationary relay, s=4: vehicles
config.lambda(1)=config.num_users*config.ratio_pedestrians*config.session_rate_per_UE/3600;   %sessions/s
config.lambda(2)=config.num_users*config.ratio_stationary*(1-config.relay_involvement_factor)*config.session_rate_per_UE/3600;   %sessions/s
config.lambda(3)=config.num_users*config.ratio_stationary*config.relay_involvement_factor*config.session_rate_per_UE/3600;   %sessions/s
config.lambda(4)=config.num_users*config.ratio_vehicles*config.session_rate_per_UE/3600;   %sessions/s
...
for n=1:numBS
    ...
    for flo=1:num_floors
        BS(n).map_SNR{flo}=config.tx_power_per_Hz+config.gain_BS+config.gain_UE-BS(n).map_totalloss{flo}-config.noise_density;
...
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%3.- OBTENTION OF THE TOTAL SNR AND SPEFF MAPS INCLUDING THE RELAYS.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
stats(sim).num_served_UEs_per_BS=zeros(1+ceil(config.simulation_duration/config.time_step),numBS);
stats(sim).sum_inv_speff=zeros(1+ceil(config.simulation_duration/config.time_step),numBS);
...
for time=0:config.time_step:config.simulation_duration
    ...
    if activate_debug_UE==0 && mod(time,100)==0
        fprintf('Sim:%d, Simulating time: %f Ped:%d, Stat:%d, Cars:%d\n',sim,time,num_active_UEs(1),num_active_UEs(2),num_active_UEs(4));
    end
    
    %MOBILITY OF ACTIVE UEs OF TYPE PEDESTRIAN AND VEHICLE.
    s=1; %Pedestrian UEs
    ...
    s=4; %Vehicles UEs
    for i=1:num_active_UEs(s)
        [UElist{s}{i}.pos,UElist{s}{i}.trajectory,stop_tx] = mobility_model_cars(UElist{s}{i}.pos,UElist{s}{i}.trajectory,UElist{s}{i}.speed,config.time_step,config.prob_change_inters_cars,sizeX,sizeY,pixel_size,map_trajectories_cars,trajectories_cars,map_numbers_traject_cars);
        if stop_tx==1
           %Force termination of this UE
           UElist{s}{i}.end_session_time=time;  
        end
    ...
    for s=1:4
        while time_next_session_arrival(s)<=time  %By putting the while, we allow multiple arrivals in a time step.
            ...
            switch s
                ...
                case 4
                    %Vehicle
                    %Choose position inside map_trajectories_cars
                    [pos_index(1),pos_index(2)]=ind2sub([sizeX,sizeY],list_valid_points_cars(randi(num_valid_points_cars)));
                    %Choose randomly a position inside the pixel.
                    UElist{s}{num_active_UEs(s)}.pos(1)=(pos_index(1)-1)*pixel_size+pixel_size*rand;
                    UElist{s}{num_active_UEs(s)}.pos(2)=(pos_index(2)-1)*pixel_size+pixel_size*rand;
                    ...
                    UElist{s}{num_active_UEs(s)}.speed=config.speed_cars;
                    UElist{s}{num_active_UEs(s)}.indoor=0;
                    
                    traj_number=map_numbers_traject_cars(pos_index(1),pos_index(2));
                    if traj_number>1000
                       %It is an intersection. Choose randomly between one trajectory
                       %or the other.
                       traj1=mod(traj_number,1000);
                    ...
                    UElist{s}{num_active_UEs(s)}.trajectory=traj_number;
                    UElist{s}{num_active_UEs(s)}.direction=trajectories_cars(traj_number).direction;
            end
    ...
    for s=1:4
        for i=1:num_active_UEs(s)
            ...
            if s==4
               %Car: only connected to BS
               UElist{s}{i}.speff=speff_total_BS{UElist{s}{i}.floor}(pos_index(1),pos_index(2));
               UElist{s}{i}.serving_BS=serving_BS{UElist{s}{i}.floor}(pos_index(1),pos_index(2));
            ...
            %Measure statistic of number of UEs per BS (counting only UEs
            %that are not in outage)
            if (UElist{s}{i}.speff>=config.speff_limit_outage)
                stats(sim).num_served_UEs_per_BS(time_index,UElist{s}{i}.serving_BS)=stats(sim).num_served_UEs_per_BS(time_index,UElist{s}{i}.serving_BS)+1;
                stats(sim).sum_inv_speff(time_index,UElist{s}{i}.serving_BS)=stats(sim).sum_inv_speff(time_index,UElist{s}{i}.serving_BS)+1/UElist{s}{i}.speff;
                %Add also the sum of inverse of speff? (can be useful to
                %decide how many RBs are required...)
            end
        end
    end
...
size_samples_speff=max([stats(sim).num_samples_speff(1),stats(sim).num_samples_speff(2),stats(sim).num_samples_speff(3),stats(sim).num_samples_speff(4)]);
size_samples_BS_connection_time=max([stats(sim).num_samples_BS_connection_time(1),stats(sim).num_samples_BS_connection_time(2),stats(sim).num_samples_BS_connection_time(3),stats(sim).num_samples_BS_connection_time(4)]);
size_samples_relay_connection_time=max([stats(sim).num_samples_relay_connection_time(1),stats(sim).num_samples_relay_connection_time(2),stats(sim).num_samples_relay_connection_time(3),stats(sim).num_samples_relay_connection_time(4)]);
...
for sim=1:config.num_simulations
   ...
   stats_to_store(sim).num_served_UEs_per_BS=stats(sim).num_served_UEs_per_BS;
   stats_to_store(sim).sum_inv_speff=stats(sim).sum_inv_speff;
end
...
save(config.output_file_name,'stats','statsglobal','config');
