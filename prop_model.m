...
switch model
    ...
    case 1  %UMa model
        ...
        %First compute path loss under LOS conditions:
        if d<=dBP
            L_LOS=28.0+22*log10(d3D)+20*log10(f);
        else
            L_LOS=28.0+40*log10(d3D)+20*log10(f)-9*log10(dBP*dBP+(hBS-hUT)*(hBS-hUT));
        end
        ...
        if LOS_condition==0
            ...
            %NLOS
            L_NLOS=13.54+39.08*log10(d3D)+20*log10(f)-0.6*(hUT-1.5);
            L=max(L_NLOS,L_LOS);
        end
        if indoor_condition==1
           %Add penetration losses: 
